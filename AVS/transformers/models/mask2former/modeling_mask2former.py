...
def set_topk_zero(tensor, k, probability=0.7):
    ...
    # ����Ϊ0��λ���Ը���probability��Ϊ1
    probabilities = torch.bernoulli(torch.full(output.size(), probability)).cuda()
    output = torch.logical_or(output, probabilities).float()
    # print(output)
    return output

def sigmoid_cross_entropy_loss(inputs: torch.Tensor, labels: torch.Tensor, num_masks: int, training: bool) -> torch.Tensor:
    r"""
    Args:
        inputs (`torch.Tensor`):
    ...
    if not drop:
        ...
        # criterion = FocalLoss(reduction=False)
...
class Mask2FormerLoss(nn.Module):
    ...
    def loss_masks(
        ...
        losses = {
            "loss_mask": sigmoid_cross_entropy_loss(point_logits, point_labels, num_masks, self.training),
    ...
    def forward(
        ...
        if auxiliary_predictions is not None:
            for idx, aux_outputs in enumerate(auxiliary_predictions):
                ...
                loss_dict = self.forward(masks_queries_logits, class_queries_logits, mask_labels, class_labels,prompt_class_projected)
