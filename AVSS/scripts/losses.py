...
def F10_IoU_BCELoss(pred_mask, ten_gt_masks, gt_temporal_mask_flag):
    ...
    # ! notice:
    # print(pred_mask.shape,ten_gt_masks.shape)
...
def IouSemanticAwareLoss(pred_masks, gt_mask, \
    ...
    if sa_loss_flag:
        if closer_flag:  # Loss_AVM_VV reported in the paper
            masked_av_loss = closer_loss(pred_masks, a_fea_list, v_map_list, gt_temporal_mask_flag, count_stages,
                                         mask_pooling_type, norm_fea, euclidean_flag, kl_flag)
        else:  # Loss_AVM_AV reported in the paper
            masked_av_loss = A_MaskedV_SimmLoss(pred_masks, a_fea_list, v_map_list, gt_temporal_mask_flag, count_stages,
                                                mask_pooling_type, norm_fea, threshold, euclidean_flag, kl_flag)
...
if __name__ == "__main__":
    gt_mask = torch.load("./features_ob/_Y-J232faOk.pth")
