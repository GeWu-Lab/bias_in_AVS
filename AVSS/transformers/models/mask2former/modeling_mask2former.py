...
def set_topk_zero_reverse(tensor, k, probability=0.2):
    res = tensor
    _, indices = torch.topk(tensor, k=k, largest=False)
    mask = torch.zeros_like(tensor)
    mask[indices] = 1
    mask = mask * (torch.rand_like(tensor) < probability).float()
    res.mul_(mask)
    return res


def set_topk_zero(tensor, k, probability=0.7):
    # 0.8 = 1 - 0.2
    # 找到前k个最小值及其位置
    values, indices = torch.topk(tensor, k, largest=False)

    # 创建全为1的张量
    output = torch.ones(tensor.size()).cuda()

    # 将topk位置设置为0
    output.scatter_(0, indices, 0)

    # 将设为0的位置以概率probability改为1
    probabilities = torch.bernoulli(torch.full(output.size(), probability)).cuda()
    output = torch.logical_or(output, probabilities).float()
    # print(output)
    return output

def sigmoid_cross_entropy_loss(inputs: torch.Tensor, labels: torch.Tensor, num_masks: int, mtl_loss: nn.Module, training: bool, src_idx: torch.Tensor, log: bool) -> torch.Tensor:
    r"""
    Args:
        inputs (`torch.Tensor`):
    ...
    if not drop:
        ...
        if log:
            print("logging")
            wandb.log({"loss_0":cross_entropy_loss.mean(1)[0],
                "loss_1":cross_entropy_loss.mean(1)[1],
                "loss_2":cross_entropy_loss.mean(1)[2],
                "loss_3":cross_entropy_loss.mean(1)[3],
                "loss_4":cross_entropy_loss.mean(1)[4],
                "loss_5":cross_entropy_loss.mean(1)[3]})
        ...
        # print(inputs.shape, labels.shape)
        if training:
            avg_sem = int((len(cross_entropy_loss.mean(1))/5)*2)
            mask = set_topk_zero(cross_entropy_loss.mean(1), avg_sem).unsqueeze(1)
            # print(mask.shape,cross_entropy_loss.shape)
            cross_entropy_loss = mask * cross_entropy_loss
    ...
    multitask = False
    if multitask:
        loss = mtl_loss(cross_entropy_loss.mean(1), src_idx[1]) / num_masks
        # loss = cross_entropy_loss.mean(1).sum() / num_masks
    else:
        pass
    return loss
...
class AutomaticWeightedLoss(nn.Module):
    ...
    def forward(self, x, index):
        ...
        for i, loss in enumerate(x):
            loss_sum += 0.5 / (self.params[index[i]] ** 2) * loss + torch.log(1 + self.params[index[i]] ** 2)
        return loss_sum
...
class FocalLoss(nn.Module):
    ...
    def forward(self, input, target):
        if self.logits:
            BCE_loss = nn.functional.binary_cross_entropy_with_logits(input, target, reduce=False)
        else:
            BCE_loss = nn.functional.binary_cross_entropy(input, target, reduce=False)
...
class Mask2FormerLoss(nn.Module):
    def __init__(self, config: Mask2FormerConfig, weight_dict: Dict[str, float]):
        ...
        self.mtl_loss = AutomaticWeightedLoss(100)
    ...
    def loss_labels(
        ...
        # print(pred_logits_transposed.shape) torch.Size([5, 151, 100])
        # print(target_classes.shape) torch.Size([5, 100])
    ...
    def loss_masks(
        ...
        indices: Tuple[np.array],
        num_masks: int,
        prompt_class_projected: torch.Tensor,
        log: bool,
        ...
        # print(pred_masks.shape) torch.Size([1n, 1, 96, 96])
        # print(target_masks.shape) torch.Size([1n, 1, 384, 384])
        ...
        losses = {
            "loss_mask": sigmoid_cross_entropy_loss(point_logits, point_labels, num_masks,self.mtl_loss, self.training, src_idx, log),
    ...
    def forward(
        ...
        mask_labels: List[torch.Tensor],
        class_labels: List[torch.Tensor],
        prompt_class_projected: Optional[Tensor] = None,
        ...
        masks_queries_logits_match = torch.einsum('ij,ijhw->ijhw', prompt_class_projected, masks_queries_logits)
        # indices = self.matcher(masks_queries_logits_match, class_queries_logits, mask_labels, class_labels)
        # class_queries_logits_match = torch.einsum('ij,ijk->ijk', prompt_class_projected, class_queries_logits)
        indices = self.matcher(masks_queries_logits_match, class_queries_logits, mask_labels, class_labels)
        # compute the average number of target masks for normalization purposes
        num_masks = self.get_num_masks(class_labels, device=class_labels[0].device)
        # get all the losses
        # print(num_masks.shape,masks_queries_logits.shape) [5, 100, 151]
        # print(class_labels, class_queries_logits.shape) [5, 100, 96, 96]
        losses: Dict[str, Tensor] = {
            **self.loss_masks(masks_queries_logits, mask_labels, indices, num_masks, prompt_class_projected, log),
        ...
        if auxiliary_predictions is not None:
            for idx, aux_outputs in enumerate(auxiliary_predictions):
                ...
                loss_dict = self.forward(masks_queries_logits, class_queries_logits, mask_labels, class_labels,prompt_class_projected, None, False)
...
class Mask2FormerAttention(nn.Module):
    ...
    def forward(
        ...
        attn_output = self.avs_adapt(attn_output)
...
class Mask2FormerMaskedAttentionDecoderLayer(nn.Module):
    ...
    def __init__(self, config: Mask2FormerConfig):
        ...
        if False:
            self.cross_qu_attn = nn.MultiheadAttention(self.embed_dim, self.config.num_attention_heads, self.config.dropout)
...
class Mask2FormerMaskedAttentionDecoder(nn.Module):
    ...
    def __init__(self, config: Mask2FormerConfig):
        ...
        # print("config.hidden_dim",config.hidden_dim) 256
        self.layernorm = nn.LayerNorm(config.hidden_dim)
        self.layernorm_sc = nn.LayerNorm(config.hidden_dim)
    ...
    def forward(
        ...
        prompt_class_projected: Optional[torch.Tensor] = None,
    ):
        r"""
        Args:
        ...
        # print("intermediate_hidden_states",intermediate_hidden_states.shape) [100, 5, 256]
        predicted_mask, attention_mask = self.mask_predictor(
            intermediate_hidden_states, pixel_embeddings, feature_size_list[0]
        )
        # print("predicted_mask", predicted_mask.shape,"attention_mask",attention_mask.shape) [5, 100, 96, 96] [40, 100, 144]
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            # new
            # hidden_states = 0.2 * inputs_embeds + hidden_states
            if False and prompt_class_projected is not None:
                # print(hidden_states.shape,prompt_class_projected.shape)
                hidden_states = (0.5 * hidden_states + 0.5 * prompt_time)
                                 # 0.5 * torch.einsum('ij,ijk->ijk', prompt_class_projected.permute(1,0), hidden_states))
                # hidden_states = torch.einsum('ij,ijk->ijk', prompt_class_projected.permute(1,0), hidden_states)
        ...
        # print(len(intermediate),intermediate[0].shape,len(intermediate_mask_predictions),intermediate_mask_predictions[0].shape) 10 torch.Size([100, 5, 256]) 10 torch.Size([5, 100, 96, 96])
...
class Mask2FormerTransformerModule(nn.Module):
    ...
    def forward(
        ...
        prompt_class_projected: Tensor = None,
        ...
        if prompt_features_projected is not None:
            query_features = query_features + prompt_features_projected
            # query_features = prompt_features_projected
        ...
        decoder_output = self.decoder(
            ...
            output_attentions=output_attentions,
            return_dict=True,
            prompt_class_projected=prompt_class_projected
        )
...
class Mask2FormerForUniversalSegmentation(Mask2FormerPreTrainedModel):
    ...
    def get_loss_dict(
        ...
        prompt_class_projected: Tensor = None,
        prompt_features_projected: Tensor = None,  # crossmodal prompt
    ) -> Dict[str, Tensor]:
    ...
    def forward(
        ...
        for decoder_output in outputs.transformer_decoder_intermediate_states:
            # print("decoder_output.transpose(0, 1)", decoder_output.transpose(0, 1).shape) torch.Size([5, 100, 256])
            class_prediction = self.class_predictor(decoder_output.transpose(0, 1))
            # print("class_prediction",class_prediction.shape) torch.Size([5, 100, 151])
            class_queries_logits += (class_prediction,)
        masks_queries_logits = outputs.masks_queries_logits
        # torch.save({"class":class_queries_logits[-1], "mask":masks_queries_logits[-1]}, "../place_holder/project/AVSS_m2f/debias_in_pth/"+vid+".pth")
        # below
        # dict_bias_logits = torch.load("../place_holder/project/AVSS_m2f/debias_in_pth/0.1540/"+vid+".pth")

        # class_queries_logits[-1] = class_queries_logits[-1]*2 - dict_bias_logits["class"]
